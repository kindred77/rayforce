...
/* Download CTA: resolve the latest release's platform binary and point the
 * Download button straight at it, with an OS-aware label. Progressive
 * enhancement — the button already links to the /releases/latest page, so it
 * works with no JS, on Windows/mobile (no native build yet), or if every
 * source is unreachable. Mirrors the stars widget: ungh.cc primary (CDN-cached,
 * no rate limit), GitHub API fallback, 1-hour localStorage cache. ungh leaves
 * asset `name` null, so we match on the download URL string, which carries the
 * platform token (linux-x86_64 / darwin-arm64) for either source. */
(function () {
  ...
  /* Which prebuilt target fits this visitor. Only Linux x86_64 and macOS
   * (arm64) ship today; Windows/mobile/unknown fall through to the releases
   * page. navigator.platform can't tell Apple Silicon from Intel, but the only
   * macOS artifact is arm64, so 'darwin' is the right (and only) match. */
  ...
  fetch('https://ungh.cc/repos/' + REPO + '/releases/latest')
    ...
    .catch(() => {
      /* Leave the button pointing at /releases/latest — still a working path. */
    });
})();
